...
from src.config import OpenAIStuff, OtherConfig
...
def initialize_rag():
    ...
    try:
        ...
        def_config = OtherConfig()

        doc_processor = DocumentProcessor(
            chunk_size=def_config.chunk_size,
            chunk_overlap=def_config.chunk_overlap,
        )
        vector_store = VectorStore()

        # Use default URLs
        urls = def_config.default_urls

        # Process documents
        documents = doc_processor.load_main(urls)

        # Create vector store
        vector_store.create(documents)

        # Build graph
        graph_builder = GraphBuilder(retriever=vector_store.get(), llm_model=llm)
        graph_builder.build()
